 ...
 * Cria tabelas e insere dados iniciais (idempotente — pode correr várias vezes).
 */
  ...
  try {
    ...
    const topicos = {
      ...
      3: [
        ...
        'Introdução a scripts automatizados',
      ],
      4: [
        'Exercícios semanais',
    ...
    for (const [ordemModulo, titulos] of Object.entries(topicos)) {
      const { rows } = await client.query(
        'SELECT id FROM modulos WHERE ordem=$1', [ordemModulo]
      );
      if (!rows[0]) continue;
    ...
    console.log('');
    console.log('👑 Próximo passo — criar admin:');
